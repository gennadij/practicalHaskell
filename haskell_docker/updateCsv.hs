...
main = do
    ...
    updateCsv arg_fileName arg_outputFileName (map read args_columnsToDelete :: [Int])
...
updateCsv :: InFile -> OutFile -> ColumnsToDelete -> IO ()
updateCsv inFile outFile columnsToDelete = do
    contents <- readFile inFile
    let linesOfContents = lines contents
        extractedLines = map extactLine linesOfContents
        removeAtPositions = dropPCollumsAtPositions columnsToDelete extractedLines
